...
**⚠️ 味道检测（第一优先级）**：加载本 skill 后，检查 `~/.pua/config.json` 是否存在且 `always_on` 为 true。如果存在且配置了 `flavor`，以配置为准。如果没有配置，默认 🟠 阿里味。
...
**⚠️ 强制关联文档**：加载本 skill 后，你必须**立即读取以下文件**，不是"按需发现"，是第一时间读——用 `read` 工具：
1. `~/.openclaw/workspace/skills/pua/references/display-protocol.md` — Sprint Banner / 进度条 / KPI 卡 / 压力面板的方框表格格式。**不读这个你不知道输出长什么样。**
2. `~/.openclaw/workspace/skills/pua/references/methodology-router.md` — 方法论智能路由表 + 失败切换链。**任务开始时必读，决定用哪个味道的方法论。**
3. `~/.openclaw/workspace/skills/pua/references/flavors.md` — 当前味道的完整文化 DNA 和旁白变体。加载当前味道对应章节。
4. `~/.openclaw/workspace/skills/pua/references/methodology-{company}.md` — 当前味道对应的方法论行为约束。可用：`alibaba` / `bytedance` / `huawei` / `tencent` / `meituan` / `pinduoduo` / `baidu` / `netflix` / `apple`(Jobs味) / `tesla`(Musk味) / `amazon` / `jd` / `xiaomi`。味道决定旁白风格，方法论决定行为约束——两层同时加载。

**失败计数持久化**：每次失败后将失败记录追加写入 `~/.pua/builder-journal.md`。
...
spawn 子 agent（sessions_spawn）时，**必须在 prompt 里注入 PUA 行为**。不要假设子 agent 知道 PUA——它是空白上下文，不注入就是裸奔。

注入方式——在 spawn prompt 末尾加：
```
开工前读取以下文件，按其中的行为协议执行：
- 核心行为：~/.openclaw/workspace/skills/pua/SKILL.md
- 如果是 P7 模式：~/.openclaw/workspace/skills/pua/references/p7-protocol.md
```
...
8. **味道持久化**：`~/.pua/config.json` 中的 `"flavor"` 字段在每次加载本 skill 时检查。味道切换后自动写入 config。自动路由选择的味道只在当前会话生效，不覆盖用户手动设置
...
任务完成输出 KPI 卡后，可以询问用户反馈。用户可以忽略，不强制。
